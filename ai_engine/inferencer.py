...
app = Flask(__name__)
log.info("AICoLoc AI Engine starting...")
...
def recommend():
    try:
        ...
        log.info(f"Scheduling request for pod: {pod_name}")
        log.info(f"Candidate nodes: {node_names}")

        features = []
        for node in node_names:
            metric_vec = collect_node_metrics(prom, node)  # [cpu, mem]
            pod_vec = get_pod_context_features(pod)        # [is_db, is_frontend, is_backend]
            full_vec = metric_vec + pod_vec + [0.0]        # padding to make 6
